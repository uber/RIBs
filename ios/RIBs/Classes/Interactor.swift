...
public protocol InteractorScope: class {
    ...
    // invokes these methods. In order to unit test router with a mock interactor, the mocked
    // interactor first needs to conform to the custom subclass interactor protocol, and also
    ...
    /// stream terminates after the interactor is deallocated.
...
public protocol Interactable: InteractorScope {
    ...
    ///
    /// - note: This method should be internally invoked by the corresponding router. Application
    /// code should never explicitly invoke this method.
    func activate()
...
/// Interactor defines a unit of business logic that corresponds to a router unit. An interactor
/// has a lifecycle driven by its owner router. When the corresponding router is attached to its
/// parent, its interactor becomes active. And when the router is detached from its parent, its
/// interactor's resigns active. An interactor should only perform its business logic when it's
/// currently active.
open class Interactor: Interactable {

    /// The logging category of the interactor. Defaults to 'Interactor'.
    ///
    /// - SeeAlso: Logging
    open var loggingCategory = "Interactor"
    ...
    /// The lifecycle of this interactor.
    public final var isActiveStream: Observable<Bool> { return isActiveSubject.asObservable().distinctUntilChanged() }
    ...
    ///
    /// - note: This method is driven by the attachment of this interactor's owner router. Subclass
    ///   should override this method to setup subscriptions and initial states.
    ...
    /// The interactor will become inactive.
    ///
    /// - note: This method is driven by the detachment of this interactor's owner router. Subclass
    ///   should override this method to cleanup any resources and states of the interactor.
...
public extension ObservableType {
    ...
    /// Confine the observable's subscription to the given interactor scope. The subscription is only triggered
...
public extension Disposable {
    ...
    /// Dispose the subscription based on the lifecycle of the given interactor. The subscription is disposed
    ...
    /// interactor.
    ///
    /// When using this composition, the subscription closure may freely retain the interactor itself,
    /// since the subscription closure is disposed once the interactor is deactivated, thus releasing the retain
    /// cycle before the interactor needs to be deallocated.
    ///
    /// If the given interactor is inactive at the time this method is invoked, the subscription is
